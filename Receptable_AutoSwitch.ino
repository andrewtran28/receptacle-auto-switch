...
#define RELAY_STARTUP_DELAY 500   // Time (ms) to delay before activating relay after tool has started
...
unsigned long lastToolSeenOn = 0; // Tracks the last time tool current was above threshold
bool toolIsOn = false;            // Tracks tool state
...
void loop() {
  ...
  // Detect tool state based on current sensor
  if (analogValue > effectiveThreshold) {
    toolIsOn = true;
    lastToolSeenOn = millis();   // Update the last time tool was detected ON
  } else {
    toolIsOn = false;
  }
  ...
  // Relay control logic
  if (toolIsOn) {
    // If tool is on, make sure relay is activated after startup delay
    if (relayPinState != RELAY_LOGIC) {
      delay(RELAY_STARTUP_DELAY);  // Small blocking delay is ok for startup only
      relayPinState = RELAY_LOGIC;
    }
  } else {
    // Tool is OFF, but only shut off relay if the shutoff delay has fully elapsed
    if (relayPinState == RELAY_LOGIC && millis() - lastToolSeenOn > RELAY_SHUTOFF_DELAY) {
      relayPinState = !RELAY_LOGIC; // Turn relay OFF
    }
  }
